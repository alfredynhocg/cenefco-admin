...
import { DirectorioListResponse } from '../../domain/models/directorio.model';
...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: DirectorioListResponse }
  | { type: 'error' };
...
export class Directorio {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p =>
        this.directorioService.getAll(p).pipe(
  ...
  get entries()   { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
  ...
  onFiltroActivo(event: Event): void {
  ...
  deleteEntry(id: number): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.directorioService.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminada!', 'La entrada ha sido eliminada');
