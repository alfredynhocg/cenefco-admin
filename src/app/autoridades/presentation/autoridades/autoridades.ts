...
import { AutoridadListResponse } from '../../domain/models/autoridad.model';
...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: AutoridadListResponse }
  | { type: 'error' };
...
export class Autoridades {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p =>
        this.autoridadService.getAll(p).pipe(
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.autoridadService.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminada!', 'La autoridad ha sido eliminada');
