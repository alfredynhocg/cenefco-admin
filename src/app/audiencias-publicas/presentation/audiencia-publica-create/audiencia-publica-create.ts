...
@Component({
  selector: 'app-audiencia-publica-create',
  imports: [ReactiveFormsModule, RouterLink, NgIcon, PageTitle],
  templateUrl: './audiencia-publica-create.html',
  styles: ``
})
export class AudienciaPublicaCreate {
  ...
  onAficheSelected(event: Event): void {
    ...
    reader.onload = (e) => this.afichePreview.set(e.target?.result as string);
  ...
  onSubmit(): void {
    ...
    this.audienciaService.create(payload).subscribe({
      next: () => {
        this.toast.success('¡Creada!', 'Audiencia pública registrada exitosamente');
        this.router.navigate(['/senefco/audiencias-publicas']);
      },
      error: (err: HttpErrorResponse) => {
        this.toast.error('Error', extractErrorMessage(err, 'No se pudo guardar la audiencia pública'));
