...
type ApiState =
  | { type: 'loading' }
  | { type: 'success'; response: TramiteListResponse }
  | { type: 'error' };
...
export class Tramites {
  ...
  private state = toSignal(
    toObservable(this.params).pipe(
      switchMap(p =>
        this.tramiteService.getAll(p).pipe(
  ...
  get tramites()  { const s = this.state(); return s.type === 'success' ? s.response.data : []; }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.tramiteService.delete(id).subscribe({
          next: () => {
            this.toast.success('¡Eliminado!', 'El trámite ha sido eliminado');
            this.refreshTrigger.update(n => n + 1);
          },
          error: () => this.toast.error('Error', 'No se pudo eliminar el trámite')
        });
      }
    });
  }
